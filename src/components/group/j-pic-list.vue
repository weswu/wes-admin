<template>
  <ul class="j_picture">
    ...
    <li class="j_picture_item" v-for="(item, index) in list" :key="index" v-if="list">
      <JUpload :img.src="item.url" @on-change="picChange($event, item, index)"/>
      <div class="bom">
        <el-popover
          placement="bottom"
          trigger="hover">
...
export default {
  ...
  methods: {
    ...
    picChange (e, item, index) {
      ...
      let list = this.list
      if(this.payId) pic.payId = this.payId
      pic.url = e.data.url
      pic.filename = e.data.fileName
      pic.original = e.data.original
      if (item === 'add') {
        list.push(pic)
      } else{
        list[index] = pic
